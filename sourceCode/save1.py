...
def guess_rating(rewiew):
    rewiew = rewiew.replace("<br />", " ")
    words_in_text = word_tokenize(rewiew)
    
    total_rating = 0

    for w in words_in_text:
        total_rating += words_pos_probability.get(w, 0.5)
    
    return round(total_rating / len(words_in_text))
...
print(TrainSetError / len(dataset["test"]))
print(1-(TrainSetError / len(dataset["test"])))
